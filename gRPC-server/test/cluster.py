...
if __name__ == '__main__':
    ...
    try:
        response = client.query_clustering('file', queries)
        logging.debug(response.comment_clustering_response_content)
        logging.debug('query clustering is ok')
    except Exception as ex:
        logging.error('failed to query clustering : {}'.format(ex))
        sys.exit(-1)
